...
base1 = [6.5194172859,0,0];  % need to adjust according to the lattice vector in the origin file
base2 = [-3.2597086430	 5.6459809875,0];

new_base1 = base1 * sup_x; % supercell
new_base2 = base2 * sup_y;
new_base3 = [0,0,6.4266276360];

filename='CONTCAR_21_unrotated_bulk_car.POSCAR.vasp'; %the input should be comming from shanshan's code
...
filename2 = 'unAB21-3-3.vasp';
fid2=fopen(filename2,'w')
fprintf(fid2,'21_unrotated_graphene\n')
fprintf(fid2,'1.0\n')
...
filename3 = 'data.unAB21-3-3type2';
fid3=fopen(filename3,'w')
fprintf(fid3,'# 21 unTwisted Bilayer Graphene cell with dimension 3 x 3 x 1\n')
    ...
    for j = 1:sup_y
        for k = 1:num_of_atom
            ...
            if c{1,3}(k) < 2
                ...
                fprintf(fid3,'%i\t %s\t \t%f\t %f\t %f\n',nnn,'2',c{1,1}(k)+(i-1)*base1(1)+(j-1)*base2(1),c{1,2}(k)+(j-1)*base2(2),c{1,3}(k)); 
            end
        end
    end
end

filename4 = 'map.unAB21-3-3type2';
fid4=fopen(filename4,'w');
fprintf(fid4,'%d\t %d\t %d\t %d\n',sup_x,sup_y,sup_z+1,num_of_atom);
fprintf(fid4,'Map file for 60x60x1 21 Bilayer Graphene cell.\n');
mmm=1;
k_m = 1;
