...
filename='afterrot32_vasp.xyz'; %the input should be comming from shanshan's code
...
c = textscan(fid,'%s %f %f %f',num_of_atom);

w=8; %initial posiztion, need to adjust so that
...
p1=[w,2*w,3.447195]; %end points of unit cell on layer 1
p2= p1 + vect1;
p3= p2 + vect2;
p4= p3 + vect3;

p5=[w,2*w,0]; %end points of unit cell on layer 2
p6= p5 + vect1;
p7= p6 + vect2;
p8= p7 + vect3;
...
filename2 = 'unitcell32.vasp';
fid=fopen(filename2,'w');
fprintf(fid,'32_rotated_graphene\n')
fprintf(fid,'1.0\n')
...
for i = 1:num_of_atom 
    ...
    if d(i,3) == 3.447195 && e(1,3) > 0 && f(1,3) > 0 && g(1,3) > 0 && h(1,3) > 0 
    fprintf(fid,'\t%f\t %f\t %f\n',d(i,1),d(i,2),d(i,3));
    end
...
for i = 1:num_of_atom 
    ...
    %according to the right hand rule, positive means inside
    if d(i,3) ==0 && e(1,3) > 0 && f(1,3) > 0 && g(1,3) > 0 && h(1,3) > 0 
    fprintf(fid,'\t%f\t %f\t %f\n',d(i,1),d(i,2),d(i,3));
    end
